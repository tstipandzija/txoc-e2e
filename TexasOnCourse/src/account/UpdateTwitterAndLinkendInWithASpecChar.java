...
public class UpdateTwitterAndLinkendInWithASpecChar {
	...
	public void passTwitterAndLinkedInSpecChar() {
		...
		try {
			...
			String social = Account_Page.social(driver).getText();
			System.out.println("Social = " + social);
			Assert.assertEquals(social, "Social");
			WebElement el = Account_Page.social(driver);
			JavascriptExecutor jsExec = (JavascriptExecutor) driver;
			jsExec.executeScript("arguments[0].click()", el);
			Thread.sleep(2000);
			
			Account_Page.twitter(driver).sendKeys("!”#$%&'()=?*ĐŽĆČŠ");
			Account_Page.linkedIn(driver).sendKeys("”!#$%&'()=?*ĐŽĆČĆŠ");
